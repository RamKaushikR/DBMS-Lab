...
SQL> select Name,Gender,Category,Aff_Role,Country from Nobel
  2  where Aff_Role is not null and (Name like 'A%' or Name like 'D%' or Name like 'H%') order by Name;

NAME                           G CAT AFF_ROLE                       COUNTRY     
------------------------------ - --- ------------------------------ ----------  
Ada E Yonath                   f Che Weizmann Institute of Science  Israel      
...
SQL> select Year_Award,Category,Field,count(*) from Nobel where Country not like 'USA'
  2  group by (Year_Award,Category,Field) having count(*)>=2 order by Year_Award;
