...
select * from customers where cid in (select cid from Receipts group by Receipts.cid, Receipts.rdate having count(*) > 2);

REM 3. Display the products details that has been ordered maximum by the customers. (use ALL)
select * from products where pid in (select item from item_list group by item having count(*) >= all(select count(*) from item_list group by item));
...
group by p.pid having count(*) <= all(select count(*) from products p join item_list i on(p.pid = i.item) group by p.pid));
...
where r1.rno = ( select r2.rno from Receipts r2 join item_list i on(re.rno = i.rno) join products p on(p.pid = i.item)
where p.food = 'Meringue' group by r2.rno having count(distinct flavor) = (select count(distinct flavor) from products where food = 'Meringue'));
...
select * from customers where cid in(select cid from customers minus select cid from Receipts);
