...
SQL> insert into customers values(21,'KAUSHIK','RAM');
...
SQL> select count(count(*)) as Count_Rec from item_list where item in
  2  (select pid from products p1 where price >
  3  (select avg(price) from products p2 group by food having p1.food = p2.food)) group by rno;
...
SQL> select c.*, r.rno, r.rdate from customers c join Receipts r on(c.cid = r.cid)
  2  where r.rdate = last_day(r.rdate);
